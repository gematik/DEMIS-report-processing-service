...
public class ContextEnrichmentService {
  ...
  @Setter
  @Value("${feature.flag.context.enrichment.service}")
  ...
  public void enrichBundleWithContextInformation(Bundle bundle, String authorization) {
    if (!enrichment) {
      log.debug("context enrichment is disabled");
      return;
    }
    ...
    try {
      ...
      final String resp =
          contextEnrichmentServiceClient.getProvenanceResource(authorization, composition.getId());
