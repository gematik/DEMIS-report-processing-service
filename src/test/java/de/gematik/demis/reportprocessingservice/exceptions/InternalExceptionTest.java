...
class InternalExceptionTest {
  ...
  void testConstructorWithErrorCode() {
    ...
    String errorCodeAsString = "ERROR_CODE_123";
    when(errorCode.asString()).thenReturn(errorCodeAsString);

    InternalException exception = new InternalException(errorCode);

    assertThat(exception.getLocalizedMessage())
        .isEqualTo(String.format("Internal Error (%s)", errorCodeAsString));
